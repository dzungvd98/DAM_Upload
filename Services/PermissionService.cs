...
namespace DAM_Upload
{
    ...
    {
        private readonly DamUploadDbContext _dbContext;
        private readonly IEmailService _emailService;
        private readonly IProducerService _producerService;
        private readonly IMapper _mapper;
        private const string TOPIC = "email_topic";
        
        public PermissionService(DamUploadDbContext dbContext, IEmailService emailService, IProducerService producerService, IMapper mapper)
        {
        ...
        private async Task<IEnumerable<PermissionDto>> GetOwnedFiles(int userId, IEnumerable<PermissionDto> filePermissions)
        {
            ...
            return ownedFiles
                .Where(file => !filePermissions.Any(p => p.ResourceId == file.FileId))
        ...
        private async Task<IEnumerable<PermissionDto>> GetOwnedFolders(int userId, IEnumerable<PermissionDto> folderPermissions)
        {
            ...
            return ownedFolders
                .Where(folder => !folderPermissions.Any(p => p.ResourceId == folder.FolderId))
        ...
        private async Task GrantPermissionsToSubfoldersAndFiles(int userId, int folderId, PermissionType permissionType, string recipientEmail)
        {
            ...
            {
                await HandleFolderPermission(userId, subfolder.FolderId, permissionType, recipientEmail);
            }

            var files = await _dbContext.Set<DAM_Upload.Models.File>()
                .Where(f => f.Folder.FolderId == folderId)
                .ToListAsync();


            foreach (var file in files)
            {
                await HandleFilePermission(userId, file.FileId, permissionType, recipientEmail);
            }
        }
